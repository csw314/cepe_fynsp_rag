...
def aggregate_site_totals(site_splits: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if working.empty and _site_column(site_splits) is None:
        return [{"status": "not_evaluated", "reason": "Missing FORMEX site_planex/site_name column."}]
...
def aggregate_site_year_heatmap(site_splits: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if working.empty and _site_column(site_splits) is None:
        return [{"status": "not_evaluated", "reason": "Missing FORMEX site_planex/site_name column."}]
    grouped = (
        working.groupby(["site", "fiscal_year_normalized", "fiscal_year_number"], dropna=False)[AMOUNT_COLUMN]
        .sum()
    ...
    return [
        {
            ...
            "fiscal_year_number": int(row.fiscal_year_number) if pd.notna(row.fiscal_year_number) else None,
            "amount": float(row.amount),
            "amount_display": format_dollars(float(row.amount)),
        }
...
def aggregate_suboffice_site_dependencies(site_splits: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    grouped = (
        working.groupby(["organization", "site", "funding_level_normalized"], dropna=False)[AMOUNT_COLUMN]
        .sum()
        .reset_index(name="amount")
            ...
            records.append(
                {
                    ...
                    "dollar_change_display": format_dollars(dollar_change) if dollar_change is not None else "Not available",
...
def _generic_scope_mask(series: pd.Series) -> pd.Series:
    ...
    generic_terms = {"support", "program support", "other", "tbd", "to be determined", "n/a", "none"}
...
def site_scope_quality_findings(site_splits: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for rule_id, title, column, check in specifications:
        if column is None or column not in site_splits.columns:
            findings.append(
                {
                    "rule_id": rule_id, "finding_id": rule_id, "title": title, "status": "not_evaluated",
                    "severity": "not_evaluated", "row_count": 0, "affected_dollars": None,
                    "affected_dollars_display": "Not available", "source_submission_type": source,
        ...
        findings.append(
            {
                "rule_id": rule_id, "finding_id": rule_id, "title": title, "status": "evaluated",
                "severity": "medium", "row_count": int(mask.sum()), "affected_dollars": amount,
                "affected_dollars_display": format_dollars(amount), "source_submission_type": source,
                "details": "Deterministic scope-quality review trigger; future RAG classification may refine generic-scope labels.",
                "source_row_id_sample": site_splits.loc[mask, "source_row_id"].astype(str).head(100).tolist()
                if "source_row_id" in site_splits.columns else [],
            }
        )
    return findings
...
def build_dashboard_03_payloads(project_root: Path | None = None) -> dict[str, Any]:
    ...
    payloads = {
        "q1": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q1", question_text=QUESTION_TEXT["q1"], chart_type="ranked_bar_table", chart_title="Pit Production funding by site",
            metric_definition="Sum of FORMEX Formulated Measure by Site in Federal Site Splits, with each site's share of the Site Splits total.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["site"], value_column=AMOUNT_COLUMN, record_count=len(site_splits), data=q1_data, summary=_leader_summary(q1_data, "site"), limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Sites represented", "value": len(q1_data), "display": str(len(q1_data))}], metadata=metadata,
        ),
        "q2": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q2", question_text=QUESTION_TEXT["q2"], chart_type="site_year_heatmap_table", chart_title="Pit Production funding by site and fiscal year",
            metric_definition="Sum of FORMEX Formulated Measure by Site and fiscal year in Federal Site Splits.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["site", "fiscal_year"], value_column=AMOUNT_COLUMN, record_count=len(site_splits), data=q2_data, summary=_leader_summary(q2_data, "site-year cell"), limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Site-year cells", "value": len(q2_data), "display": str(len(q2_data))}], metadata=metadata,
        ),
        "q3": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q3", question_text=QUESTION_TEXT["q3"], chart_type="above_baseline_dependency_table", chart_title="Site dependence on above-baseline funding",
            metric_definition="Baseline, ROT, UFR, total funding, and above-baseline share by Site from Federal Site Splits.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["site", "funding_level"], value_column=AMOUNT_COLUMN, record_count=len(site_splits), data=q3_data, summary=_leader_summary(q3_data, "above-baseline total", "above_baseline"), limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Sites with above-baseline funding", "value": sum((row.get("above_baseline") or 0) != 0 for row in q3_data), "display": str(sum((row.get("above_baseline") or 0) != 0 for row in q3_data))}], metadata=metadata,
        ),
        "q4": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q4", question_text=QUESTION_TEXT["q4"], chart_type="suboffice_site_matrix", chart_title="Sub-office to site funding dependencies",
            metric_definition="Sum of FORMEX Formulated Measure by Sub Office Number, Site, and funding level in Federal Site Splits, with distinct sub-office counts per site.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["organization", "site", "funding_level"], value_column=AMOUNT_COLUMN, record_count=len(site_splits), data=q4_data, summary=_leader_summary(q4_data, "organization-site dependency"), limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Organization-site relationships", "value": len(q4_data), "display": str(len(q4_data))}], metadata=metadata,
        ),
        "q5": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q5", question_text=QUESTION_TEXT["q5"], chart_type="yoy_change_table", chart_title="Site-level funding cliffs and surges",
            metric_definition="Year-over-year dollar and percent change by Site in Federal Site Splits; zero prior-year values use new funding/not meaningful percent states.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["site", "fiscal_year"], value_column=AMOUNT_COLUMN, record_count=len(site_splits), data=q5_data,
            summary=f"{sum(row.get('review_flag', False) for row in q5_data)} site-year cells are surge, cliff, or new-funding review triggers.", limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Review-trigger cells", "value": sum(row.get("review_flag", False) for row in q5_data), "display": str(sum(row.get("review_flag", False) for row in q5_data))}], metadata=metadata,
        ),
        "q6": make_payload(
            dashboard_id=DASHBOARD_ID, dashboard_title=DASHBOARD_TITLE, question_id="q6", question_text=QUESTION_TEXT["q6"], chart_type="scope_quality_scorecard", chart_title="Site-level scope and traceability findings",
            metric_definition="Column-tolerant deterministic checks for missing/generic scope, program request, site, WBS, and BNR on Federal Site Splits.", source_submission_type=SITE_SPLITS_SUBMISSION_TYPE, row_filter=row_filter,
            grouping_columns=["rule_id"], value_column=f"{AMOUNT_COLUMN} affected dollars", record_count=len(site_splits), data=q6_data,
            summary=f"{sum(row.get('row_count', 0) for row in q6_data if row.get('status') == 'evaluated')} site rows are represented by deterministic completeness review triggers.", limitations=limitations, lineage=source_row_lineage(site_splits),
            metric_cards=[{"label": "Evaluated finding categories", "value": sum(row.get("status") == "evaluated" for row in q6_data), "display": str(sum(row.get("status") == "evaluated" for row in q6_data))}], metadata=metadata,
        ),
    }
