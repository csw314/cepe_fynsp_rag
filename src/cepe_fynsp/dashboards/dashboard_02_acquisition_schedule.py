...
def aggregate_acquisition_type_by_funding_level(crosscuts: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    grouped = (
        working.groupby(["acquisition_type_label", "funding_level_normalized"], dropna=False)[
            AMOUNT_COLUMN
        ]
        .sum()
...
def aggregate_largest_acquisition_lines(crosscuts: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    grouped = (
        working.groupby(
            ["acquisition_id", "acquisition_name", "acquisition_type", "funding_level_normalized"],
            dropna=False,
        )[AMOUNT_COLUMN]
        .sum()
        .reset_index(name="amount")
    ...
    totals = grouped.groupby(
        ["acquisition_id", "acquisition_name", "acquisition_type"], dropna=False
    )["amount"].sum()
...
def _schedule_frame(crosscuts: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    working["start_date_parsed"] = pd.to_datetime(
        working["acquisition_start_date"], errors="coerce"
    )
...
def aggregate_acquisition_timeline_funding(crosscuts: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for values, group in working.groupby(group_columns, dropna=False):
        ...
        funding_years = sorted(
            int(value) for value in group["fiscal_year_number"].dropna().unique().tolist()
        )
        outside = 0
        ...
        records.append(
            {
                ...
                if outside or pd.isna(start) or pd.isna(end)
                else "within_schedule",
            }
        )
...
def detect_acquisition_schedule_exceptions(
    ...
    specifications = [
        (
            "AQ001",
            "Acquisition type populated but acquisition ID blank",
            ["acquisition_type", "acquisition_id"],
        ),
        (
            "AQ002",
            "Acquisition ID populated but acquisition name blank",
            ["acquisition_id", "acquisition_name"],
        ),
        (
            "AQ003",
            "Acquisition-related row missing start date",
            ["acquisition_id", "acquisition_type", "acquisition_start_date"],
        ),
        (
            "AQ004",
            "Acquisition-related row missing end date",
            ["acquisition_id", "acquisition_type", "acquisition_end_date"],
        ),
        (
            "AQ005",
            "Acquisition start date is after end date",
            ["acquisition_start_date", "acquisition_end_date"],
        ),
        (
            "AQ006",
            "Funding fiscal year falls outside acquisition schedule",
            ["acquisition_start_date", "acquisition_end_date", "fiscal_year_number"],
        ),
        (
            "AQ007",
            "High-dollar acquisition row lacks schedule metadata",
            [
                "acquisition_id",
                "acquisition_type",
                "acquisition_start_date",
    ...
    for rule_id, title, required in specifications:
        ...
        if missing:
            ...
            continue
        acquisition_related = ~blank_mask(crosscuts["acquisition_id"]) | ~blank_mask(
            crosscuts["acquisition_type"]
        )
        if rule_id == "AQ001":
            mask = ~blank_mask(crosscuts["acquisition_type"]) & blank_mask(
                crosscuts["acquisition_id"]
            )
        elif rule_id == "AQ002":
            mask = ~blank_mask(crosscuts["acquisition_id"]) & blank_mask(
                crosscuts["acquisition_name"]
            )
        else:
            ...
            if rule_id == "AQ003":
                ...
                mask = (
                    starts.notna()
                    & ends.notna()
                    & ((years < starts.dt.year) | (years > ends.dt.year))
                )
            else:
                mask = (
                    acquisition_related
                    & (starts.isna() | ends.isna())
...
def aggregate_li_tec_li_opc_site_year(site_splits: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    working["acquisition_type"] = (
        working["acquisition_type"].astype("string").str.strip().str.upper()
    )
    working["site"] = working[site_column].map(
        lambda value: display_value(value, "Unspecified site")
    )
    grouped = (
        working.groupby(["site", "fiscal_year_normalized", "acquisition_type"], dropna=False)[
            AMOUNT_COLUMN
        ]
        .sum()
        .reset_index(name="amount")
...
def _summary(data: list[dict[str, Any]], empty: str, key: str = "amount") -> str:
    ...
    label = (
        leader.get("acquisition_id")
        or leader.get("site")
        or leader.get("program_request")
        or leader.get("acquisition_type")
    )
...
def build_dashboard_02_payloads(project_root: Path | None = None) -> dict[str, Any]:
    ...
    payloads = {
        "q1": make_payload(
            ...
            chart_title="Pit Production funding by acquisition type and funding level",
            ...
            grouping_columns=["acquisition_type", "funding_level"],
            value_column=AMOUNT_COLUMN,
            record_count=len(crosscuts),
            data=q1_data,
            summary=_summary(q1_data, "No acquisition-type funding rows were available."),
            limitations=limitations,
            lineage=source_row_lineage(crosscuts),
            metric_cards=[
                {
                    "label": "Crosscuts acquisition funding",
        ...
        "q2": make_payload(
            ...
            chart_title="Largest Pit Production acquisition lines",
            ...
            record_count=len(crosscuts),
            data=q2_data,
            summary=_summary(q2_data, "No acquisition lines were available."),
            limitations=limitations,
            lineage=source_row_lineage(crosscuts),
            metric_cards=[
                {
                    "label": "Acquisition line rows",
                    "value": len(q2_data),
        ...
        "q3": make_payload(
            ...
            chart_type="schedule_table",
            chart_title="Acquisition dates and funding-year alignment",
            ...
            data=q3_data,
            summary=_summary(
                q3_data, "No acquisition schedule rows were available.", "funding_amount"
            ),
            limitations=limitations,
            lineage=source_row_lineage(crosscuts),
            metric_cards=[
                {
                    "label": "Scheduled acquisition lines",
                    "value": len(q3_data),
                    "display": str(len(q3_data)),
                }
            ],
            metadata=metadata,
        ),
        "q4": make_payload(
            ...
            chart_type="exception_table",
            chart_title="Acquisition schedule metadata exceptions",
            ...
            record_count=len(crosscuts),
            data=q4_data,
            summary=f"{sum(row.get('row_count', 0) for row in q4_data if row.get('status') == 'evaluated')} acquisition-row exceptions were flagged for analyst review.",
            limitations=limitations,
            lineage=source_row_lineage(crosscuts),
            metric_cards=[
                {
                    "label": "High-severity checks",
                    "value": sum(
                        row.get("severity") == "high" and row.get("row_count", 0) > 0
                        for row in q4_data
                    ),
                    "display": str(
                        sum(
        ...
        "q5": make_payload(
            ...
            chart_title="LI TEC and LI OPC funding by site and year",
            ...
            row_filter={**sites_filter, "acquisition_type": ["LI TEC", "LI OPC"]},
            grouping_columns=["site", "fiscal_year", "acquisition_type"],
            value_column=AMOUNT_COLUMN,
            record_count=len(site_splits),
            data=q5_data,
            summary=_summary(q5_data, "LI TEC and LI OPC could not be evaluated from Site Splits."),
            limitations=limitations
            + [
                "No Crosscuts-to-Site-Splits join is invented when Site Splits acquisition fields are weak or absent."
            ],
            lineage=source_row_lineage(site_splits),
            metric_cards=[
                {
                    "label": "LI TEC / LI OPC cells",
                    "value": len(q5_data),
        ...
        "q6": make_payload(
            ...
            chart_title="Above-baseline acquisition requests with priority signals",
            ...
            data=q6_data,
            summary=_summary(q6_data, "No above-baseline acquisition requests were available."),
            limitations=limitations
            + ["Priority signals identify review targets; they do not establish request merit."],
            lineage=source_row_lineage(
                crosscuts.loc[crosscuts["funding_level_normalized"].isin(["ROT", "UFR"])]
            ),
            metric_cards=[
                {
                    "label": "Material requests",
                    "value": sum(row.get("is_material", False) for row in q6_data),
                    "display": str(sum(row.get("is_material", False) for row in q6_data)),
                }
            ],
            metadata=metadata,
        ),
