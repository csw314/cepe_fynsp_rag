...
def _crosscuts() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "acquisition_type": ["LI TEC", None, "LI OPC"], "acquisition_id": ["A", None, "B"],
            "acquisition_name": ["Alpha", None, "Bravo"], "acquisition_start_date": ["2028-01-01", None, "2031-01-01"],
            "acquisition_end_date": ["2030-01-01", None, "2030-01-01"], "fiscal_year_number": [2029, 2028, 2028],
            "funding_level_normalized": ["Baseline", "ROT", "UFR"], "formulated_measure": [100.0, 200.0, 300.0],
            "source_row_id": ["a", "b", "c"], "program_request": ["A", "B", "C"], "program_priority": [1, 2, 3], "doe_priority_tier": [1, 2, 1],
        }
    )
...
def test_li_tec_li_opc_filter_uses_only_matching_types() -> None:
    sites = _crosscuts().assign(site_planex=["LANL", "SRS-SRNS", "LANL"], fiscal_year_normalized=["FY2029", "FY2028", "FY2028"])
...
def test_dashboard_02_build_writes_payloads(tmp_path) -> None:
    ...
    frame = _crosscuts().rename(columns={
        "acquisition_type": "Acquisition Type", "acquisition_id": "Acquisition ID", "acquisition_name": "Acquisition Name",
        "acquisition_start_date": "Acquisition Start Date", "acquisition_end_date": "Acquisition End Date",
        "program_request": "Program Request", "program_priority": "Program Priority", "doe_priority_tier": "DOE Priority Tier",
        "formulated_measure": "Formulated Measure",
    }).drop(columns=["fiscal_year_number", "funding_level_normalized", "source_row_id"]).assign(**{
    ...
    pd.concat([frame, site_rows], ignore_index=True).to_csv(raw / "formex.csv", sep="\t", encoding="utf-16", index=False)
    ...
    output = tmp_path / "data" / "curated" / "dashboard_payloads" / "dashboard_02_acquisition_schedule"
    assert manifest["dashboard_id"] == "dashboard_02_acquisition_schedule"
    assert json.loads((output / "q1_acquisition_type_by_funding_level.json").read_text())["question_id"] == "q1"
